...
AdjR2PCATraining = zeros(pcaMaxDim,1);

R2PCATest = zeros(pcaMaxDim,1);
AdjR2PCATest = zeros(pcaMaxDim,1);
...
for i = 1:1:length(countryList)
    ...
    for dim = 1:pcaMaxDim
        ...
        bPCA = regress(Y,Xinput);
        YpredPCA = Xinput*bPCA;
        
        bPCAArray{dim} = bPCA;

        % Training R2 and AdjR2
        R2PCATraining(dim) = Rsq(YpredPCA,Y);
        AdjR2PCATraining(dim) = adjRsq(YpredPCA,Y,length(Y),dim);
    end
    ...
    for dim = 1:pcaMaxDim
        ...
        YpredPCA = Xinput*bPCAArray{dim};
        

        % Training R2 and AdjR2
        R2PCATest(dim) = Rsq(YpredPCA,Y);
        AdjR2PCATest(dim) = adjRsq(YpredPCA,Y,length(Y),dim);
    end
    
    [AdjR2PCATest,sortIdx] = sort(AdjR2PCATest,'descend');
    bestPCA = sortIdx(1);
    
    R2Test(i,5) = R2PCATest(bestPCA);
